...
class BiternionMixture:
    ...
    def plot_pdf_circle(self, img, xvals, pdf, ypred_rad=None, ytrue_rad=None, show_legend=True,
        ...
        if ytrue_rad is not None:
            ax_pdf.axvline(ytrue_rad, ymin=0.54, ls='dashed', color='red', linewidth=4, label='ground truth')
        if ypred_rad is not None:
            ax_pdf.axvline(ypred_rad, ymin=0.54, ls='dashed', color=pred_color, linewidth=4, label='prediction')
        if show_legend:
            ax_pdf.legend(fontsize=25, loc=1, framealpha=1.0)

        return fig

    def visualize_detections_on_circle(self, x, y_true=None, show_legend=True, save_path=None):

        n_images = x.shape[0]

        if save_path is not None:
            if not os.path.exists(save_path):
                print("creating directory for saved visualizations : %s" % save_path)
        ...
        for i in range(0, n_images):
            ...
            if save_path is None:
                ...
                save_img_path = os.path.join(save_path,'frame_%d.png'%i)
                fig.savefig(save_img_path, dpi=500)
